...
```bash
npm test             # 266 unit, wiring and asset checks — fast, no browser
npm run lint
...
`npm test` — 266 checks across six files, no browser, under a second.
...
`npm run test:dom` — 72 checks in real headless Chrome, driven over the DevTools
