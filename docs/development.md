...
```bash
npm test             # 199 unit, wiring and asset checks — fast, no browser
npm run lint
...
`npm test` — 199 checks across six files, no browser, under a second.
...
`npm run test:dom` — 43 checks in real headless Chrome, driven over the DevTools
