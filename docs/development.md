...
```bash
npm test             # 251 unit, wiring and asset checks — fast, no browser
npm run lint
...
`npm test` — 251 checks across six files, no browser, under a second.
...
`npm run test:dom` — 59 checks in real headless Chrome, driven over the DevTools
