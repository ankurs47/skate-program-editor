...
```bash
npm test             # 256 unit, wiring and asset checks — fast, no browser
npm run lint
...
`npm test` — 256 checks across six files, no browser, under a second.
...
`npm run test:dom` — 65 checks in real headless Chrome, driven over the DevTools
